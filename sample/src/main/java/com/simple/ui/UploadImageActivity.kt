...
@RefRegister(title=R.string.general_upload_image_sample,desc=R.string.general_upload_image_sample_desc,category = R.string.general_upload_category,priority = 2)
class UploadImageActivity : BaseSampleAppCompatActivity() {
    ...
    fun formUpload() {
        ...
        if (imageItems != null && imageItems.size > 0) {
            for (i in imageItems.indices) {
                fileMap["uploadimg"] = File(imageItems[i].path)
            }
        }
        uploadImg(fileMap)
    }
    ...
    fun uploadImg(files: Map<String, File>) {
            ...
            files[key]?.let {
                ...
                builder.addFormDataPart(key, it.name, object: ProgressRequestBody(imageBody){
                    override fun onUpload(progress: Long, contentLength: Long, done: Boolean) {
                        ...
                        ThreadTaskExecutor.create(this@UploadImageActivity).post({
